...
impl Image {
    ...
    pub fn data<Col: Color>(&self) -> &[Col] {
    ...
    pub fn data_mut<Col: Color>(&mut self) -> &mut [Col] {
        assert!(Col::color_type() == self.color_type);
        unsafe {
    ...
    pub fn get<Col: Color>(&self, x: u32, y: u32) -> Col {
        assert!(Col::color_type() == self.color_type);
        self.data()[self.index(x, y)]
    }

    pub fn pixels_mut<Col: Color>(&mut self) -> Vec<Vec<&mut Col>> {
        assert!(Col::color_type() == self.color_type);
    ...
    pub fn set<Col: Color>(&mut self, x: u32, y: u32, value: Col) {
        assert!(Col::color_type() == self.color_type);
        let index = self.index(x, y);
        self.data_mut()[index] = value;
    }

    pub fn clear<Col: Color>(&mut self, color: Col) {
        assert!(Col::color_type() == self.color_type);
...
mod test {
    ...
    fn load() {
        ...
        assert!(image.data::<RGBA8>().iter().all(|&value: &RGBA8| value == color));
    }
}
