...
impl PrimitiveBuilder {
    ...
    pub fn vertices(mut self, vertices: Vec<Vertex>) -> Self {
        match &mut self.geometry {
            ...
            _ => self.geometry = Geometry::Triangles(Triangle::new(vertices, vec![])),
        }
        self
    }
    ...
    pub fn indices(mut self, indices: Vec<u8>) -> Self {
        match &mut self.geometry {
            ...
            _ => self.geometry = Geometry::Triangles(Triangle::new(vec![], indices)),
        }
        self
    }
    ...
    pub fn index_size(mut self, index_size: usize) -> Self {
        match &mut self.geometry {
            Geometry::Triangles(triangles) => {
                triangles.index_size = index_size;
            }
            _ => {
                let mut triangle = Triangle::new(vec![], vec![]);
                triangle.index_size = index_size;
...
impl Primitive {
    ...
    pub fn unit_sphere() -> Self {
        Self::builder().sphere(Vec3::default(), 1.0).build()
    }
    ...
    pub fn primitives<'m>(
        ...
        match &self.geometry {
            Geometry::Triangles(triangles) => triangles.primitives(transform, material, model),
            Geometry::Sphere(sphere) => sphere.primitives(transform, material, model),
        }
    }
}
