...
use crate::{Bvh, BvhLight, Color, Integrator, Irradiance, Ray};
...
impl Integrator for Scratcher {
    fn trace(&self, ray: Ray, bvh: &Bvh, lights: &[BvhLight], depth: u32) -> Option<Color> {
        ...
        let albedo_color = hit.primitive.get_color(&hit);
        ...
        let uv = hit.primitive.get_uv(&hit);
        ...
        for light in lights {
            ...
            if is_light {
                let intensity = light.light.get_intensity(light.trs, &hit.point);
                let ir = Irradiance::new(intensity, &hit, light_dir, n, -ray.dir, albedo_color, uv);
                pixel_color += hit.primitive.get_radiance(&ir);
            }
        } // end iterate light
        ...
        if let Some(reflection_intensity) = self.trace(reflection_ray, bvh, lights, depth + 1) {
