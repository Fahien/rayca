...
impl IndexMut<usize> for Mat4 {
    fn index_mut<'a>(&'a mut self, i: usize) -> &'a mut f32x4 {
        &mut self.values[i]
    }
}
...
impl From<&Inversed<&Trs>> for Mat4 {
    fn from(inv_trs: &Inversed<&Trs>) -> Self {
        Mat4::from_scale(&inv_trs.source.scale)
            * (Mat4::from_rotation(&inv_trs.source.rotation)
                * Mat4::from_translation(&inv_trs.source.translation))
    }
}
